                        ...
                        {aplicantes[oferta.id] && aplicantes[oferta.id].length > 0 ? (
                          aplicantes[oferta.id].map((aplicante, index) => (
                            <li key={index} className="border-b py-2">
                              <div><strong>Nombre:</strong> {aplicante.nombre} {aplicante.apellido}</div>
                              <div><strong>Cédula/Pasaporte:</strong> {aplicante.cedulaPasaporte}</div>
                              <div><strong>Teléfono:</strong> {aplicante.telefono}</div>
                              <div><strong>Correo:</strong> {aplicante.correo}</div>
                              <div><strong>Fecha de Nacimiento:</strong> {aplicante.fechaNacimiento}</div>
                              <div><strong>Profesión:</strong> {aplicante.profesion}</div>
                              <div><strong>Nivel Académico:</strong> {aplicante.nivelAcademico}</div>
                              <div><strong>Comentarios:</strong> {aplicante.comentarios}</div>
                              <div><strong>Currículo:</strong> <a href={aplicante.curriculo} target="_blank" className="text-blue-500">Ver Currículo</a></div>
                              <div><strong>Fecha de Aplicación:</strong> {format(new Date(aplicante.fechaAplicacion.seconds * 1000), "dd/MM/yyyy HH:mm:ss")}</div>
                              <button
                                onClick={() => eliminarAplicante(oferta.id, aplicante)}
                                className="mt-2 text-red-600 hover:text-red-800 py-2 px-3 rounded-md"
                              >
                                <AiOutlineDelete /> Eliminar Aplicante
                              </button>
                            </li>
                          ))
                        ) : (
